    ...
    //has the user resized the client area?
      ...
      break;
		 ...
		 case WM_DESTROY:
			 ...
			 //g_pRaven->GetMap()->GetNavGraph()->SaveTransitionMatrixInFile();
                    ...
                    int       iCmdShow)
{
 ...
 //the window class structure
  ...
  {  		 
    ...
    bool bDone = false;
	int numberOfLoops = 0;
	int numberOfLoopsBeforeMatrixSave = 200000;

	/*Raven_Bot* bot = new Raven_Bot(g_pRaven, Vector2D(0, 0));
	Raven_PathPlanner* pathPlanner = new Raven_PathPlanner(bot);*/
    ...
    while(!bDone)
    {
      while( PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) ) 
      {
        if( msg.message == WM_QUIT ) 
        {
          // Stop loop if it's a quit message
			g_pRaven->GetMap()->GetReinforcementLearning()->SaveTransitionMatrixInFile();
	        bDone = true;
        } 
	  ...
	  //g_pRaven->GetMap()->GetReinforcementLearning()->Train(pathPlanner);
	  

	  /*if (numberOfLoops++ % numberOfLoopsBeforeMatrixSave == 0)
	  {
		  //g_pRaven->GetMap()->GetReinforcementLearning()->SaveTransitionMatrixInFile();
	  }*/
