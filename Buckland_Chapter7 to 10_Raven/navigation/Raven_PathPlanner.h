...
protected:
  ...
  //the given position
  virtual int   GetIndexOfClosestNodeToPosition(Vector2D pos)const;


  //smooths a path by removing extraneous edges. (may not remove all
  //extraneous edges)
  virtual void  SmoothPathEdgesQuick(Path& path);

  //smooths a path by removing extraneous edges. (removes *all* extraneous
  //edges)
  virtual void  SmoothPathEdgesPrecise(Path& path);
  ...
  virtual void  GetReadyForNewSearch();



public:

  virtual ~Raven_PathPlanner();

  Raven_PathPlanner(Raven_Bot* owner);


  Raven_Map::NavGraph& GetNavGraph(){return m_NavGraph;}

  //returns the closest visible and unobstructed graph node to
  //the given position
  virtual Raven_Map::NavGraph::NodeType Raven_PathPlanner::GetClosestNodeToPosition(Vector2D pos)const;
  ...
  virtual double      GetCostToNode(unsigned int NodeIdx)const;
  ...
  //trigger found
  virtual double      GetCostToClosestItem(unsigned int GiverType)const;
  ...
  virtual int        CycleOnce()const;

  virtual Vector2D   GetDestination()const{return m_vDestinationPos;}
  virtual void       SetDestination(Vector2D NewPos){m_vDestinationPos = NewPos;}
  ...
  virtual Vector2D  GetNodePosition(int idx)const;
