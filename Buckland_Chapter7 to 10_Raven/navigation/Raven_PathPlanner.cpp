...
double Raven_PathPlanner::GetCostToNode(unsigned int NodeIdx)const
{
  ...
  int nd = GetIndexOfClosestNodeToPosition(m_pOwner->Pos());
...
Raven_PathPlanner::Path Raven_PathPlanner::GetPath()
{
  ...
  int closest = GetIndexOfClosestNodeToPosition(m_pOwner->Pos());
...
//------------------------ GetIndexOfClosestNodeToPosition ---------------------------
//
//  returns the index of the closest visible graph node to the given position
//-----------------------------------------------------------------------------
int Raven_PathPlanner::GetIndexOfClosestNodeToPosition(Vector2D pos)const
{
  double ClosestSoFar = MaxDouble;
...
//  returns the index of the closest visible graph node to the given position
//-----------------------------------------------------------------------------
Raven_Map::NavGraph::NodeType Raven_PathPlanner::GetClosestNodeToPosition(Vector2D pos)const
{
	double ClosestSoFar = MaxDouble;
	Raven_Map::NavGraph::NodeType   ClosestNode;
...
{ 
  ...
  //find the closest visible node to the bots position
  int ClosestNodeToBot = GetIndexOfClosestNodeToPosition(m_pOwner->Pos());
  ...
  int ClosestNodeToTarget = GetIndexOfClosestNodeToPosition(TargetPos);
