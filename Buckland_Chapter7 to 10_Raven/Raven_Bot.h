...
protected:
  ...
  //if the bot is using the reinforcement learning
  bool								m_bReinforcementBot;
...
public:
  ...
  virtual void         Write(std::ostream&  os)const{/*not implemented*/}
  virtual void         Read (std::ifstream& is){/*not implemented*/}
  ...
  virtual int           Health()const{return m_iHealth;}
  virtual int           MaxHealth()const{return m_iMaxHealth;}
  virtual void          ReduceHealth(unsigned int val);
  virtual void          IncreaseHealth(unsigned int val);
  virtual void          RestoreHealthToMaximum();

  virtual int           Score()const{return m_iScore;}
  virtual void          IncrementScore(){++m_iScore;}

  virtual Vector2D      Facing()const{return m_vFacing;}
  virtual double        FieldOfView()const{return m_dFieldOfView;}

  virtual bool          isPossessed()const{return m_bPossessed;}
  virtual bool          isDead()const{return m_Status == dead;}
  virtual bool          isAlive()const{return m_Status == alive;}
  virtual bool          isSpawning()const{return m_Status == spawning;}
  
  virtual void          SetSpawning(){m_Status = spawning;}
  virtual void          SetDead(){m_Status = dead;}
  virtual void          SetAlive(){m_Status = alive;}

  virtual bool          GetReinforcement() { return m_bReinforcementBot; }

  //returns a value indicating the time in seconds it will take the bot
  //to reach the given position at its current speed.
  virtual double        CalculateTimeToReachPosition(Vector2D pos)const;

  //returns true if the bot is close to the given position
  virtual bool          isAtPosition(Vector2D pos)const;


  //interface for human player
  virtual void          FireWeapon(Vector2D pos);
  virtual void          ChangeWeapon(unsigned int type);
  virtual void          TakePossession();
  ...
  virtual Raven_Game* const                  GetWorld(){return m_pWorld;}
  virtual Raven_Steering* const              GetSteering(){return m_pSteering;}
  virtual Raven_PathPlanner* const           GetPathPlanner(){return m_pPathPlanner;}
  ...
  virtual Raven_Bot* const                   GetTargetBot()const{return m_pTargSys->GetTarget();}
  virtual Raven_WeaponSystem* const          GetWeaponSys()const{return m_pWeaponSys;}
  virtual Raven_SensoryMemory* const         GetSensoryMem()const{return m_pSensoryMem;}
