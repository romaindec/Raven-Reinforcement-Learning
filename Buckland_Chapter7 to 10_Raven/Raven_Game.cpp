...
//-----------------------------------------------------------------------------
void Raven_Game::AddBots(unsigned int NumBotsToAdd, bool reinforcementBots)
{
	while (NumBotsToAdd--)
	{
		Raven_Bot* rb;
		if (reinforcementBots)
		{	//create a bot. (its position is irrelevant at this point because it will
			//not be rendered until it is spawned)
			rb = new Raven_Learning_Bot(this, Vector2D());
		}
		else
		{
...
#ifdef LOG_CREATIONAL_STUFF
		debug_con << "Adding bot with ID " << ttos(rb->ID()) << " , is learning bot : " << reinforcementBots;
#endif
	}
}
...
{  
  ...
  { 
    AddBots(0/*script->GetInt("NumBots")*/, false);
